...
export async function PATCH(request: Request) {
  ...
  let activityType = action, activityDetails = '';

  if (action === 'marketing_assign') {
    if (!canManageDepartment(ctx, 'Marketing')) return NextResponse.json({ error: 'Only the Marketing lead or Super Admin can assign marketing employees' }, { status: 403 });
    payload.assigned_marketing_id = body.employeeId ?? null; payload.assigned_department = 'Marketing';
    if (body.employeeId) payload.status = 'Assigned';
    activityDetails = `Marketing assignment changed to ${body.employeeId ?? 'unassigned'}`;
  } else if (action === 'marketing_update') {
    if (!canEditAssignedRecord(ctx, lead.assigned_marketing_id) || (!ctx.isAdmin && ctx.department !== 'Marketing')) return NextResponse.json({ error: 'Only the assigned marketing employee, Marketing lead or Super Admin can update this lead' }, { status: 403 });
    ...
    if (body.nextAction !== undefined) payload.next_action = body.nextAction || null;
    if (body.nextActionAt !== undefined) payload.next_action_at = body.nextActionAt || null;
    activityDetails = body.notes || body.pipelineStage || 'Marketing lead updated';
  } else if (action === 'transfer_to_sales') {
    if (!canManageDepartment(ctx, 'Marketing')) return NextResponse.json({ error: 'Only the Marketing lead or Super Admin can transfer leads to Sales' }, { status: 403 });
    ...
    payload.assigned_department = 'Sales'; payload.assigned_dept_head_id = body.salesLeadId ?? null; payload.assigned_sales_id = null; payload.status = 'Qualified';
    payload.qualified_at = new Date().toISOString(); activityDetails = 'Lead handed over from Marketing to Sales';
  } else if (action === 'sales_assign') {
    if (!canManageDepartment(ctx, 'Sales') || lead.assigned_department !== 'Sales') return NextResponse.json({ error: 'Only the Sales lead or Super Admin can assign sales employees' }, { status: 403 });
    payload.assigned_sales_id = body.employeeId ?? null; if (body.employeeId) payload.status = 'Assigned';
    activityDetails = `Sales assignment changed to ${body.employeeId ?? 'unassigned'}`;
  } else if (action === 'sales_update') {
    if (!canEditAssignedRecord(ctx, lead.assigned_sales_id) || (!ctx.isAdmin && ctx.department !== 'Sales')) return NextResponse.json({ error: 'Only the assigned sales employee, Sales lead or Super Admin can update this lead' }, { status: 403 });
    if (body.status !== undefined && !isLeadStatus(body.status)) return NextResponse.json({ error: 'Invalid lead status' }, { status: 400 });
    payload.status = body.status ?? lead.status; payload.sales_notes = body.notes ?? lead.sales_notes;
    if (body.salesPipelineStage !== undefined) payload.sales_pipeline_stage = String(body.salesPipelineStage);
    ...
    if (body.nextAction !== undefined) payload.next_action = body.nextAction || null;
    if (body.nextActionAt !== undefined) payload.next_action_at = body.nextActionAt || null;
    if (body.status === 'Contacted') payload.last_contacted_at = new Date().toISOString();
    if (body.status === 'Qualified' && !lead.qualified_at) payload.qualified_at = new Date().toISOString();
    if (body.status === 'Converted') payload.converted_at = new Date().toISOString();
    activityDetails = body.notes || body.salesPipelineStage || 'Sales lead updated';
