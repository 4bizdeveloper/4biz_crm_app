        ...
        {data.map((d, i) => (
          <g key={i}>
            <circle
              ...
              cy={getY(d.value)}
              r="3.5"
              className="fill-white stroke-[#0284c7] stroke-2 hover:r-5 transition-all"
            />
            {i === Math.floor(data.length / 2) && (
              <circle
                ...
                cy={getY(d.value)}
                r="6"
                className="fill-[#0284c7] stroke-white stroke-2 animate-pulse"
              />
            )}
            <text
              x={getX(i)}
  ...
  return (
    <div className="w-full h-48 flex flex-col justify-between pt-2">
          ...
          return (
            <div key={idx} className="relative z-10 flex-1 flex flex-col items-center h-full justify-end group">
              ...
              <div className="w-full max-w-[36px] bg-slate-100 rounded-lg h-full flex items-end p-1 overflow-hidden border border-slate-200">
                <div
                  className="w-full bg-gradient-to-t from-[#0284c7] to-[#38bdf8] rounded-md transition-all duration-700 shadow-md group-hover:brightness-110"
      ...
      <div className="flex justify-between gap-2 px-1 pt-2.5">
        {data.map((item, idx) => (
          <span key={idx} className="flex-1 text-center text-[10px] font-medium text-slate-600 truncate">
            {item.label}
          </span>
        ))}
      </div>
...
export default function OverviewPage() {
  ...
  return (
    <div className="min-h-screen bg-transparent text-slate-700 p-3 sm:p-5 md:p-6 font-sans space-y-4 md:space-y-6">
      ...
      {/* HEADER BAR */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white/90 backdrop-blur-md p-4 sm:p-5 rounded-2xl border border-slate-200 shadow-sm">
        <div className="text-center sm:text-left">
          <h1 className="text-xl sm:text-2xl font-bold text-slate-900 tracking-tight">Sales & Operations Overview</h1>
        ...
        <div className="flex items-center justify-center sm:justify-end gap-2.5">
          ...
          <button
            ...
            className="p-2 bg-slate-100 hover:bg-slate-200 text-sky-600 border border-slate-200 rounded-xl transition-all cursor-pointer shadow-xs flex items-center justify-center"
            title="Refresh Data"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        ...
        {/* Card 1: Total Leads */}
        <div className="bg-white/90 backdrop-blur-md p-4 rounded-2xl border border-slate-200 shadow-sm flex flex-col items-center justify-center text-center group hover:border-slate-300 transition-all">
          ...
          <div className="text-xl sm:text-2xl font-extrabold text-slate-900 tracking-tight my-1">
            {stats.totalLeads} <span className="text-xs font-semibold text-sky-600">Active</span>
          </div>
          <p className="text-[11px] text-slate-500 font-medium flex items-center gap-1 justify-center">
            <TrendingUp className="w-3 h-3 text-sky-600" /> +14.2% from last cycle
          </p>
        </div>
        ...
        <div className="bg-white/90 backdrop-blur-md p-4 rounded-2xl border border-slate-200 shadow-sm flex items-center justify-around text-left hover:border-slate-300 transition-all">
          <div className="flex flex-col items-start">
            <span className="text-xs font-medium text-slate-500 block mb-0.5">Win & Close Rate</span>
            <span className="text-base font-bold text-slate-900 block">Target: 80%</span>
            <span className="text-[11px] text-sky-600 font-medium flex items-center gap-1 mt-0.5">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 md:gap-5">
        ...
        <div className="lg:col-span-7 bg-white/90 backdrop-blur-md p-4 sm:p-5 rounded-2xl border border-slate-200 shadow-sm flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between mb-3">
              <h2 className="text-xs font-bold text-slate-900 uppercase tracking-wider">Pipeline Operational Velocity</h2>
              <MoreHorizontal className="w-3.5 h-3.5 text-slate-400 cursor-pointer" />
            </div>
            <ModernXYLineChart data={pipelineTrendData} />
          </div>
          <div className="flex items-center justify-center gap-6 pt-3 border-t border-slate-200 text-xs text-slate-600">
            <span className="flex items-center gap-1.5">
              <span className="w-2 h-2 rounded-full bg-sky-600" /> Realized Output
            </span>
            <span className="flex items-center gap-1.5">
        ...
        <div className="lg:col-span-5 bg-white/90 backdrop-blur-md p-4 sm:p-5 rounded-2xl border border-slate-200 shadow-sm flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between mb-3">
              <h2 className="text-xs font-bold text-slate-900 uppercase tracking-wider">Volume by Operations Stage</h2>
              <MoreHorizontal className="w-3.5 h-3.5 text-slate-400 cursor-pointer" />
            </div>
          ...
          <div className="pt-3 border-t border-slate-200 flex justify-between items-center text-xs text-slate-500">
            ...
            <span className="text-sky-600 font-semibold">{stats.totalLeads + stats.activeProjects} Total Items</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-4 md:gap-5">
        ...
        <div className="lg:col-span-4 bg-white/90 backdrop-blur-md p-4 sm:p-5 rounded-2xl border border-slate-200 shadow-sm space-y-3 flex flex-col justify-between">
          <div className="flex items-center justify-between border-b border-slate-200 pb-2.5">
            ...
            <span className="text-[9px] font-bold uppercase bg-emerald-50 text-emerald-700 px-2 py-0.5 rounded-full border border-emerald-200">
              Live Desk
            </span>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2.5">
            <div className="p-3 bg-red-50/50 rounded-xl border border-red-100 flex flex-col items-center justify-center text-center">
              ...
              <span className="text-xl font-black text-red-700 mt-1">{stats.ticketsByPriority['Urgent'] || 0}</span>
            </div>

            <div className="p-3 bg-orange-50/50 rounded-xl border border-orange-100 flex flex-col items-center justify-center text-center">
            ...
            <div className="p-3 bg-sky-50/50 rounded-xl border border-sky-100 flex flex-col items-center justify-center text-center">
              <span className="text-[10px] font-bold text-sky-600 uppercase tracking-wider">Medium</span>
              <span className="text-xl font-black text-sky-700 mt-1">{stats.ticketsByPriority['Medium'] || 0}</span>
            </div>

            <div className="p-3 bg-slate-50 rounded-xl border border-slate-200 flex flex-col items-center justify-center text-center">
              <span className="text-[10px] font-bold text-slate-600 uppercase tracking-wider">Low</span>
              <span className="text-xl font-black text-slate-800 mt-1">{stats.ticketsByPriority['Low'] || 0}</span>
            </div>
          </div>
        </div>

        {/* Live Operational Timeline */}
        <div className="lg:col-span-4 bg-white/90 backdrop-blur-md p-4 sm:p-5 rounded-2xl border border-slate-200 shadow-sm flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between border-b border-slate-200 pb-2.5 mb-3">
              <h2 className="text-xs font-bold text-slate-900 flex items-center gap-1.5 uppercase tracking-wider">
                <Clock className="w-3.5 h-3.5 text-sky-600" /> Recent Operational Logs
              </h2>
            </div>
              ...
              {stats.recentActivities.length === 0 ? (
                ...
                stats.recentActivities.map((act, index) => (
                  ...
                  >
                    ...
                    <span className="text-[10px] font-semibold text-slate-500 shrink-0">{act.time}</span>
                  </div>
                ))
              )}
            </div>
        ...
        <div className="md:col-span-2 lg:col-span-4 bg-white/90 backdrop-blur-md p-4 sm:p-5 rounded-2xl border border-slate-200 shadow-sm space-y-3 flex flex-col justify-between">
          <div className="flex items-center justify-between border-b border-slate-200 pb-2.5">
            <h2 className="text-xs font-bold text-slate-900 flex items-center gap-1.5 uppercase tracking-wider">
              <Users className="w-3.5 h-3.5 text-purple-600" /> Workforce Allocation
            </h2>
            <span className="text-xs font-bold text-slate-900">{stats.totalEmployees} Active</span>
          </div>
